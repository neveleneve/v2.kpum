            ...
            <div class="table-responsive">
                <table class="table table-hover table-bordered">
                    ...
                    <tbody class="text-nowrap">
                        ...
                        @php
                        $jumlahsuara += $item->vote;
                        @endphp
                        @endforeach
                        @forelse ($calon as $item)
                        <tr>
                            <td class="text-center">
                                <a class="btn btn-sm btn-warning font-weight-bold" href="{{ route('viewcalon', ['id'=>$item->id]) }}">
                                    <i class="fas fa-eye"></i>
                                </a>
                            </td>
                            <td>{{ $item->no_urut }}</td>
                            <td>{{ ucwords($item->ketua) . ' - ' . ucwords($item->wakil) }}</td>
                            <td class="text-right">{{ $item->vote }} Suara ({{ $item->vote == 0 ? 0 : round($item->vote / $jumlahsuara, 4) * 100 }}%)</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="3">
                    ...
                    <tfoot>
                        @if (count($calon)>0)
                        <tr>
                            <td colspan="3" class="text-right font-weight-bold">Jumlah Suara</td>
                            <td class="text-right">
                                {{ $jumlahsuara }} Suara
                            </td>
                        </tr>
                ...
                <form action="{{ route('addcalon') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <input class="form-control mb-3" type="text" name="no_urut" id="no_urut" required onkeypress="return isNumberKey(event)">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-lg-6 col-md-12 col-sm-12">
                            <label for="ketua">Nama Calon Ketua</label>
                            <input class="form-control mb-3" type="text" name="ketua" id="ketua" required onkeypress="return isCharKey(event)">
                            <label for="nimketua">Nomor Induk Mahasiswa Calon Ketua</label>
                            <input class="form-control mb-3" type="text" name="nimketua" id="nimketua" required onkeypress="return isNumberKey(event)">
                            ...
                            <input class="form-control mb-3" type="text" name="angkatanketua" id="angkatanketua" required onkeypress="return isNumberKey(event)">
                        </div>
                        <div class="col-lg-6 col-md-12 col-sm-12">
                            <label for="wakil">Nama Calon Wakil Ketua</label>
                            <input class="form-control mb-3" type="text" name="wakil" id="wakil" required onkeypress="return isCharKey(event)">
                            <label for="nimwakil">Nomor Induk Mahasiswa Calon Wakil Ketua</label>
                            <input class="form-control mb-3" type="text" name="nimwakil" id="nimwakil" required onkeypress="return isNumberKey(event)">
                            ...
                            <input class="form-control mb-3" type="text" name="angkatanwakil" id="angkatanwakil" required onkeypress="return isNumberKey(event)">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12">
